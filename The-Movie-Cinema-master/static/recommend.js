...
var myAPI = 'e63331fcc31fb53f7279e464df4b2549'  // global string to be consistent with future usages elsewhere
$(function() {
  $('#movie_list').css('display','none');
  $('#autoComplete').blur(function() {
    $('#movie_list').css('display','none');
  });
  // 在我们在输入字段中输入一些内容之前，按钮将被禁用
...
function load_details(my_api_key,search,isQuerySearch){
  if(isQuerySearch) {
    url = 'https://api.themoviedb.org/3/search/movie?api_key='+my_api_key+'&query='+search+'&language=zh-CN';
  }
  else {
    url = 'https://api.themoviedb.org/3/movie/' + search + '?api_key='+my_api_key+'&language=zh-CN';
  }
  $.ajax({
    type: 'GET',
    url:url,
...
function get_movie_details(movie_id,my_api_key,movie_title,movie_title_org) {
  $.ajax({
    type:'GET',
    url:'https://api.themoviedb.org/3/movie/'+movie_id+'?api_key='+my_api_key+'&language=zh-CN',
...
// 将所有详细信息传递给python的flask，以便使用imdb id显示和抓取电影评论
function show_details(movie_details,movie_title,my_api_key,movie_id,movie_title_org){
  ...
  // 调用 `get_movie_cast` 获取查询电影的主要演员
  movie_cast = get_movie_cast(movie_id,my_api_key);
  
  // 调用 `get_individual_cast` 获取单个演员的详细信息
  ind_cast = get_individual_cast(movie_cast,my_api_key);

  // 调用 `get_recommendations` 从TMDB API获取给定电影id的推荐电影
...
function get_individual_cast(movie_cast,my_api_key) {
    ...
    for(var cast_id in movie_cast.cast_ids){
      $.ajax({
        type:'GET',
        url:'https://api.themoviedb.org/3/person/'+movie_cast.cast_ids[cast_id]+'?api_key='+my_api_key+'&language=zh-CN',
        async:false,
...
function get_movie_cast(movie_id,my_api_key){
    ...
    $.ajax({
      type:'GET',
      url:"https://api.themoviedb.org/3/movie/"+movie_id+"/credits?api_key="+my_api_key+"&language=zh-CN",
      async:false,
  ...
  function get_recommendations(movie_id, my_api_key) {
    ...
    $.ajax({
      type: 'GET',
      url: "https://api.themoviedb.org/3/movie/"+movie_id+"/recommendations?api_key="+my_api_key+"&language=zh-CN",
      async: false,
